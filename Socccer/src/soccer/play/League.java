...
import soccer.utility.PlayerDatabase;
import soccer.utility.PlayerDatabaseException;
import soccer.utility.Settings;
...
public class League {
  public static void main(String[] args) {
    ...
    try {
      ...
      // setTeamStats() separate from showBestTeam() as used in other places too (e.g.
      // getDescription())
    ...
    for (Team homeTeam : theTeams) {
      for (Team awayTeam : theTeams) {
        if (homeTeam != awayTeam) {
          ...
          theGames.add(
              new Game(homeTeam, awayTeam, LocalDateTime.now().plusDays(daysBetweenGames)));
        }
      }
    }
    ...
    for (Game currGame : theGames) {
      ...
      if (theResult.isDrawnGame()) {
        currGame
            .getHomeTeam()
            .incPointsTotal(
                Settings.DRAWN_GAME_POINTS); // Another way to do it currGame vs. theResult
  ...
  public String getLeagueAnnouncement(Game[] theGames) {
    ...
    Period thePeriod =
        Period.between(
            theGames[0].getTheDateTime().toLocalDate(),
    ...
    for (int i = 0; i < theTeams.length; i++) {
      ...
      for (Team currAwayTeam : theTeams) {
        ...
        if (currHomeTeam != currAwayTeam) {
          ...
          theGrid[rowNum][colNum] =
              new DisplayString(" X "); // Mark with X as team doesn't play itself
        }
      }

      // Add last two columns to current row (team points and goals)
      theGrid[rowNum][colNum + 1] =
          new DisplayString(new Integer(currHomeTeam.getPointsTotal()).toString());
      theGrid[rowNum][colNum + 2] =
          new DisplayString(new Integer(currHomeTeam.getGoalsTotal()).toString());
    }
    return theGrid;
  }
  ...
  public void outputTextLeagueGrid(IDisplayDataItem[][] dataGrid) {
    ...
    // It is simple to iterate throught the dataGrid and get each item. However, this
    // outputTextLeagueGrid() method
    // will output a grid using monospaced text and therefore needs to calculate how wide to make
    // each column.
    ...
    int totalLength =
        0; // totalLength helps calculate length needed for horizontal line between columns
    ...
    // Create horizontal line of correct length. One extra added to line length for last vertical
    // bar
    String separatorLine =
        new String(new char[totalLength + (numCols * numExtraCharsPerColum) + 1])
            .replace("\0", "-");
    ...
    for (IDisplayDataItem[] theRow : dataGrid) {
      ...
      for (int j = 0; j < theRow.length; j++) {
        ...
        System.out.print(
            "| "
                + theRow[j].getDisplayDetail()
                + new String(new char[extraChars]).replace("\0", " ")
                + " ");
      }
