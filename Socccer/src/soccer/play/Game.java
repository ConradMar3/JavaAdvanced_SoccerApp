...
public class Game implements IDisplayDataItem {
    ...
    for (int i = 1; i <= Settings.GAME_LENGTH; i++) {
      ...
      if (Math.random() > Settings.GAME_EVENT_FREQUENCY) {
        ...
        */
        ...
        // currPlayer = event1.changePlayer()?currTeam.getPlayerArray()[playerIndex = (playerIndex +
        // 1 + ((int)(Math.random()*(playerArrLength-1))))%playerArrLength]:currPlayer; // Always
        // change
        ...
        ArrayList<Player> currPlayerList =
            new ArrayList(Arrays.asList(currEvent.getTheTeam().getPlayerArray()));
        currPlayerList.remove(currPlayer);
        currEvent.setThePlayer(
            currEvent.changePlayer()
                ?
                // currEvent.getTheTeam().getPlayerArray()[(int)(Math.random() *
                // currEvent.getTheTeam().getPlayerArray().length)]:
                currPlayerList.get((int) (Math.random() * currPlayerList.size()))
                :
                // currEvent.changePlayer()?currTeam.getPlayerArray()[playerIndex = (playerIndex + 1
                // + ((int)(Math.random()*(playerArrLength-1))))%playerArrLength]:
                currPlayer);
  ...
  public String getDescription(boolean showEvents) {
    ...
    returnString.append(
        " (" + theResult.getHomeTeamScore() + " - " + theResult.getAwayTeamScore() + ") \n");

    // Add description of the events
    if (showEvents) {
      for (GameEvent currEvent : this.getEvents()) {
        returnString.append(
            currEvent.getBallPos()
                + " : "
                + currEvent
                + "after "
                + currEvent.getTheTime()
                + " mins by "
